        ...
        const insightResponse = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              { 
                role: 'system', 
                content: `你是洞察分析师。根据提取的数据生成6-8条“亮点洞察”。风格：${getStyleDescription(style)}。

输出规范（非常重要）：
- label：2-6字短标题，不要包含数字/单位/标点结尾
- value：只能是“一个核心数字 + 单位”，例如“8024.12 元 / 121 笔 / 572 小时 / 94%”，不要写其他文字
- subtext：一句完整口语化解释（≤30字），必须能单独读通；不要出现“元/笔/小时”等悬空单位；不要把 value 直接贴在句子开头

如果无法从数据确定单位，请用“次/条/分钟”等中性单位，不要留空。` 
              },
              { 
                role: 'user', 
